    ...
    const validatedArray = clientArray.map((item, index) => {
        ...
        for (const refKey in referenceStructure) {
            if (!(refKey in item)) {
                ...
                warnings.push(`Array '${key}' index ${index}: Added missing key '${refKey}'.`);
            } else {
...
function syncFiles(referenceFilePath, clientFilePath) {
    ...
    if (referenceData._inputs && clientData._inputs) {
        const { updatedInputs, warnings: inputWarnings } = syncInputs(referenceData._inputs, clientData._inputs);
    ...
    for (const key of Object.keys(referenceData)) {
        ...
        } else if (Array.isArray(referenceData[key]) && Array.isArray(clientData[key])) {
            const { validatedArray, warnings: arrayWarnings } = validateArray(referenceData[key], clientData[key], key);
            clientData[key] = validatedArray;
            warnings.push(...arrayWarnings);
        } else if (typeof referenceData[key] === 'object' && typeof clientData[key] === 'object') {
